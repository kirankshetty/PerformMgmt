...
- **KRAs/Goals Self Review**: Employees can self-review their KRA/Goal targets set by managers. The page shows 5 status cards: Pending Goals (overdue/rejected past periods), New Goals (current period), Goals to be Submitted (saved drafts), Pending Approval (submitted), and Approved. Employees can rate themselves and add comments, save as draft, and submit for manager approval. Uses `kra_goal_reviews` table with workflow statuses (draft/submitted/approved/rejected). Review periods are dynamically generated based on the KRA's review frequency (Daily/Weekly/Monthly/Quarterly/Half Yearly/Annual), the date the target was set by the manager, and the appraisal cycle date range. Only periods from the target set date onwards are shown, with exact date intervals displayed per review frequency.
- **Manager Reports**: Score Card (KPI-level aggregated scores with drill-down), Activity Report (flat KPI review table per employee per period), Trend Report (KPI-level data with current + 3 previous period history). All use date overlap filtering and period-aware target values from `kpi_target_history`.
- **Employee Reports**: Activity Report (employee's own KPI reviews with date filtering, summary cards for total reviews/weightage/achievement), Leaderboard (company-wide performance ranking with AI-powered insights chat using OpenAI). The leaderboard compares all active employees/managers in the same company, ranks by overall weighted achievement score, and includes an AI chat panel for personalized improvement suggestions.
